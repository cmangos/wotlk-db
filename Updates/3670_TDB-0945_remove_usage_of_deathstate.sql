...
UPDATE `creature_template` SET `UnitFlags` = `UnitFlags`|4096 WHERE `entry` IN (6172,6177); -- 17542,17768 have that flag too in wotlk-db maybe needed (pre wotlk (UNIT_FLAG_PVP))
...
DELETE FROM `creature_loot_template` WHERE `item` = 6846 AND `entry` = 6866; -- Defias Bodyguard
...
REPLACE INTO `creature_addon` (`guid`, `auras`) SELECT `guid`, '31261' FROM `creature` WHERE `guid` IN ( -- identical spawns with tbc-db

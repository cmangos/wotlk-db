...
UPDATE `creature_template` SET `SpellList`=0 WHERE `Entry`=36678;

-- UnitClass 4 supplies the abomination's native energy power type. Script
-- bindings remain owned by the core SQL rather than the world database.
...
DELETE FROM `reference_loot_template_names` WHERE `entry` IN (34234,34258);

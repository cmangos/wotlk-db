...
DELETE FROM `pool_template` WHERE `entry` = 1699; -- classic-db
INSERT INTO `pool_template` (`entry`, `max_limit`, `description`) VALUES (1699, 1, 'Abominable Greench (13602)');
DELETE FROM `pool_creature_template` WHERE `pool_entry` = 1699;
INSERT INTO `pool_creature_template` (`id`, `pool_entry`, `chance`, `description`) VALUES (13602, 1699, 0, 'Abominable Greench (13602)');

UPDATE `creature` SET `spawntimesecsmin` = 180, `spawntimesecsmax` = 180 WHERE `guid` = 80927 AND `id` = 1949; -- Align with classic-db

UPDATE `creature_template_addon` SET `b2_1_pvp_state` = 1, `emote` = 233 WHERE `entry` = 7804; -- Add missing Emote for Trenton Lighthammer (Might be Script)

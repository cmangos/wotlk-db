...
-- supported maps (33,34,36,43,47,574,575,576,578) - ToDo (48,70,90,109,129,189,209,229,230,249,269,289,309,329,349,389,409,429,469,509,529,531,532,533,534,540,542,543,544,545,546,547,548,550,552,553,554,555,556,557,558,560,564,565,568,574,575,576,578,580,585,595,599,600,601,602,603,604,608,615,616,619,624,631,632,649,650,658,668,724)

DROP TABLE IF EXISTS tmp_creature;
DROP TABLE IF EXISTS tmp_gameobject;

CREATE TABLE tmp_creature (SELECT guid, id, map FROM creature WHERE map IN (33,34,36,43,47,574,575,576,578));
CREATE TABLE tmp_gameobject (SELECT guid, id, map FROM gameobject WHERE map IN (33,34,36,43,47,574,575,576,578));
